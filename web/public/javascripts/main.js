...
var FiltersService = (function () {
    ...
    FiltersService.prototype.currentFilters = function () {
        var output = [];
    ...
    FiltersService.prototype.episodesFilter = function (episode) {
        var output = true;
        for (var filterKey in this._currentFilters) {
            if (this._currentFilters.hasOwnProperty(filterKey) && this._currentFilters[filterKey] !== null) {
    ...
    FiltersService.prototype.addFilter = function (input, filter) {
        var filterInfo = filter(input);
        this._currentFilters[filterInfo.name] = filterInfo;
    };
    FiltersService.prototype.removeFilter = function (filter) {
        if (filter) {
    ...
    FiltersService.prototype.clearFilters = function () {
        this._currentFilters = {};
    };
    FiltersService.prototype.filtersApplied = function () {
...
angular.module("main", ['ngAnimate'])
    ...
    function ($http, filters) {
        ...
        $http.get('/json/episodes/recent').success(function (data) {
            self.episodes = data;
        });
    }
])
    ...
    function ($parse, $animate, $rootScope) {
        return {
            ...
            compile: function ($element, attr) {
                var animClass = $parse(attr.animClick, null, true);
    ...
    function (filters, $scope, $timeout, $http) {
        var self = this;
        $http.get('/json/channels').success(function (data) {
            self.channels = data;
        });
